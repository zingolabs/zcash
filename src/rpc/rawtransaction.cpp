...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            GETRAWTRANSACTION_HELP + HelpExampleCli("getrawtransaction", "\"mytxid\"") + HelpExampleCli("getrawtransaction", "\"mytxid\" 1") + HelpExampleRpc("getrawtransaction", "\"mytxid\", 1") + HelpExampleCli("getrawtransaction", "\"mytxid\" 0 \"myblockhash\"") + HelpExampleCli("getrawtransaction", "\"mytxid\" 1 \"myblockhash\""));
...
UniValue verifytxoutproof(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1) {
        HelpSections help_sections =
            HelpSections(__func__)
                .set_usage("\"proof\"")
                .set_description("Verifies that a proof points to a transaction in a block, returning the transaction it commits to\n"
                                 "and throwing an RPC error if the block is not in our best chain")
                .set_arguments("1. \"proof\"    (string, required) The hex-encoded proof generated by gettxoutproof")
                .set_result("[\n"
                            "\"txid\"      (string)  The txid or txids which the proof commits to, or empty array if the proof invalid \n ]");
        throw runtime_error(
            help_sections.combine_sections());
    }
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1) {
        HelpSections help_sections =
            HelpSections(__func__)
                .set_usage("\"hexstring\"")
                .set_description("Return a JSON object representing the serialized, hex-encoded transaction.")
                .set_arguments("1. \"hex\"      (string, required) The transaction hex string")
                .set_result("{\n"
