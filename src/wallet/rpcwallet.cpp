...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1) {
        HelpSections help_sections =
            HelpSections(__func__)
                .set_usage("\"account\"")
                .set_description("DEPRECATED. Returns the current Zcash address for receiving payments to this account.")
                ...
                .set_result("\"zcashaddress\"   (string) The account Zcash address");
        throw runtime_error(
            help_sections.combine_sections());
    }
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2) {
        HelpSections help_sections =
            HelpSections(__func__)
                .set_usage("\"zcashaddress\" \"account\"")
                .set_description("DEPRECATED. Sets the account associated with the given address.")
                .set_arguments(
                    "1. \"zcashaddress\"  (string, required) The Zcash address to be associated with an account.\n"
                    "2. \"account\"         (string, required) MUST be set to the empty string \"\" to represent the default account. Passing any other string will result in an error.")
                .set_examples("\"t14oHp2v54vfmdgQ3v3SNuQga8JKHTNi2a1\" \"tabby\"");
        throw runtime_error(
            help_sections.combine_sections());
    }
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "4. includeWatchonly (boolean, optional, default=false) Include transactions to watchonly addresses (see 'importaddress')\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            ...
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain) transaction between accounts,"
            "and not associated with an address, transaction id or block. 'send' and 'receive' transactions are associated with an address, transaction id and "
            "block details\n"
            ...
            CURRENCY_UNIT + ". This is negative for the 'send' category, and for the 'move' category "
                            "for moves outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            ...
            CURRENCY_UNIT + ". This is negative and only available for the "
                            ...
                            "from (for receiving funds, positive amounts), or went to (for sending funds, negative amounts).\n"
...
UniValue listaccounts(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "2. includeWatchonly (boolean, optional, default=false) Include balances in watchonly addresses (see 'importaddress')\n"
            "\nResult:\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"includeWatchonly\"    (boolean, optional, default=false) Whether to include watchonly addresses in balance calculation and details[]\n"
            "\nResult:\n"
            "{\n"
            "  \"status\" : \"mined|waiting|expiringsoon|expired\",    (string) The transaction status, can be 'mined', 'waiting', 'expiringsoon' or 'expired'\n"
            ...
            MINOR_CURRENCY_UNIT + "\n"
                                  ...
                                  "      \"nullifiers\" : [\n"
                                  "           string       (string) Nullifiers of input notes\n"
                                  "      ],\n"
                                  "      \"commitments\" : [\n"
                                  "           string       (string) Note commitments for note outputs\n"
                                  "      ],\n"
                                  "      \"macs\" : [\n"
                                  "           string       (string) Message authentication tags\n"
                                  "      ],\n"
...
UniValue z_gettotalbalance(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "2. includeWatchonly (boolean, optional, default=false) Also include balance in watchonly addresses (see 'importaddress' and 'z_importviewingkey')\n"
            "\nResult:\n"
            "{\n"
...
UniValue z_sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 4) {
        HelpSections help_sections =
            HelpSections(__func__)
                .set_usage("\"fromaddress\" [{\"address\":... ,\"amount\":...},...] ( minconf ) ( fee )")
                .set_description("Send multiple times. Amounts are decimal numbers with at most 8 digits of precision."
                ...
                .set_arguments("1. \"fromaddress\"         (string, required) The transparent or shielded address to send the funds from.\n"
                ...
                .set_result("\"operationid\"          (string) An operationid to pass to z_getoperationstatus to get the result of the operation.")
                .set_examples("\"t1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" '[{\"address\": \"ztfaW34Gj9FrnGUEf833ywDVL62NWXBM81u6EQnM6VR45eYnXhwztecW1SjxA7JrmAXKJhxhj3vDNEpVCQoSvVoSpmbhtjf\", \"amount\": 5.0}]'")
                .set_examples("\"ANY_TADDR\" '[{\"address\": \"t1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", \"amount\": 2.0}]'");
        throw runtime_error(
            help_sections.combine_sections());
    }
...
UniValue z_shieldcoinbase(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 4) {
        HelpSections help_sections =
            HelpSections(__func__)
                .set_usage("\"fromaddress\" \"tozaddress\" ( fee ) ( limit )")
                .set_description("Shield transparent coinbase funds by sending to a shielded zaddr.  This is an asynchronous operation and utxos"
                ...
                .set_arguments("1. \"fromaddress\"         (string, required) The address is a taddr or \"*\" for all taddrs belonging to the wallet.\n"
                ...
                .set_result("{\n"
                            ...
                            "}")
                .set_examples("\"t1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"ztfaW34Gj9FrnGUEf833ywDVL62NWXBM81u6EQnM6VR45eYnXhwztecW1SjxA7JrmAXKJhxhj3vDNEpVCQoSvVoSpmbhtjf\"");
        throw runtime_error(
            help_sections.combine_sections());
    }
