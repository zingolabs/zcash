...
bool LookupHost(std::string strHost, std::vector<CNetAddr>& vIP, unsigned int nMaxSolutions, bool fAllowLookup)
{
    if (strHost.empty())
        return false;
...
{
    static const unsigned char pchRFC6052[12] = {0,0x64,0xFF,0x9B,0,0,0,0,0,0,0,0};
...
{
    static const unsigned char pchRFC4862[8] = {0xFE,0x80,0,0,0,0,0,0};
...
{
    static const unsigned char pchRFC6145[12] = {0,0,0,0,0,0,0,0,0xFF,0xFF,0,0};
...
bool CService::GetSockAddr(struct sockaddr* const paddr, socklen_t *addrlen) const
{
    if (IsIPv4()) {
...
CSubNet::CSubNet(const std::string &strSubnet, bool fAllowLookup)
{
    ...
    if (LookupHost(strAddress.c_str(), vIP, 1, fAllowLookup))
    {
        ...
        if (slash != strSubnet.npos) // This means '/' was found. Else -1 == -1
        {
            ...
            if (ParseInt32(strNetmask, &n)) // If valid number, assume CIDR notation /24 symtex
